...
4. The biggest problem with databases with more observations for each patients
   ...
   righ position. In particular we need the  folloqing columns
   - id
   - start
   - end
   - event
...
5. Get results easy to explain to / undertand by a clinician!

## Basic tests and funtions
...
First of all we have to ask to ourself, and to the clinicians, some questions:
...
2. What we have to do with the transplant? I.e., `event` has three levels:
   ...
   random! So, we can beleve that it is a non-informative censoring.^[In reality
   this is not really true because who stay better is on the top of the list!]
   
Moreover we have to consider, befor to start, that `sex` is a categorical
...
The proportional hazard ipotesis is not invalidated
...
### Impact of aspartate aminotransferase (`ast`) on death
  - same of age
...
not that linear. Try to transform it using the $log()$ trasformation.
...
The situtation is not much better...but we can say that there exists a line
...
### Impact of __platelet__ on death
  - same of age
...
We can try to perform a categorization. Two startegy:
  1. look at the log-linear plot and try to find a good cutpoints, but we have
    to explain how we have defined them (and "use the p-value" is not a good
    strategy)
  2. Use standard non related cutof, such as median or quartile
...
## Investigation on adjusted variables and interactions
...
Clinicians: and what about teir interaction?^[The answere here should be "if
...
## Longitudinal suvival data analayses
...
- The only triky task is to correctly manage and prepare the data. Our proposal
  take advantage of the `dplyr` functionalities
...
Here is another example in which the opionon of a clinician is mandatory, i.e.
we cannot decide if ignore outliers, which ones, etcetera
...
### priognostic model with `àscites`, `edema`, `sex`, `bili`, `ast`, `platelet`, `stage`
...
eccetera (more or less other $15 -- 30$ df).

We decide (following suggestions from @harrell2015regression) to consider
splines for any continuous variable (with 3 knots) and consider sex interactin 
